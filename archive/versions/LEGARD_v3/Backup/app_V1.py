    ...
    def Start2(self,user,day,MaxStretch):
        ...
        def Ibutton(channel):
            ...
            if btn1[0] == 0 and time.time()-btn1[7]>0.5:
                ...
                btn1[6] += 1
                info.append([btn1[6],btn1[5],btn1[2],btn1[8]])
        ...
        class MainWindow(Tk):
            def __init__(self):
                ...
                self.User = Label(self, text = "Name: " + str(user[0][0]), font = 'Helvetica 20').grid(row=1,column=1, sticky = "W")
            ...
            def close(self):
                ...
                if Box == 'yes':
                    ...
                    with open("Users/"+user[0][2]+"/"+today+".txt", "a") as f:
                        f.write("Cycle\tMax. Length\tAcceleration\tTime\n")
                        for l in info:
            ...
            def plot(self,i):
                try:
                    if btn2[3] != 0:
                        self.PackLabel()
                    self.plot1.set(xlim =(x[-1]-15, x[-1]+5), ylim =(-1, 90))
                    self.plot1.set_xlabel('Time (s)')
                    self.plot1.set_ylabel('Distance (cm)')
                    self.plot1.plot(x,y, color='green')
                    self.plot1.legend([str(round(btn1[2][-1], 2)) + 'cm/s^2'], prop={'size':40}, handletextpad=0, handlelength =0)
                    self.canvas.draw()
        ...
        def Calculations():
            while True:
                ...
                try:
                    ...
                    if btn1[0] == 0:
                        ...
                        try:
                            ...
                            a = round(2*acos(qw1[-2])*57.2957795131, 2)
                            qw1.append(qw1[-2])
                    y.append(round(2*pi*r*(a/360), 2))
                    x.append(next(index))
                    time.sleep(1)
                if btn1[1] == True:
                    ...
                    if y[-1] > btn1[5]:
                        btn1[5] = y[-1]
                    acc = (y[-1] - y[-2]) / (btn1[4]-btn1[3])**2
                    btn1[2].append(round(acc,2))
                    btn1[8].append(x[-1])
        ...
        if __name__ == '__main__':
            with Manager() as manager:
                ...
                btn1 = manager.list([0, False, manager.list([0]), 0, 0, 0, 0, time.time(), manager.list([0])])
