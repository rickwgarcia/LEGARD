        ...
        def show():
            try:
                ...
                for i,t in enumerate(T):
                    bx.text(float(t[1])+0.5, float(max(Y)) + 5, 'Rep '+str(i+1))
                    bx.text(float(t[1])+0.5, float(max(Y)) + 3, 'Max. Angle '+ str(MxAng[i][0]))
    ...
    def Start2(self,user,day,MaxStretch):
            ...
            def plot(self,i):
                ...
                try:                        
                   ...
                   # self.plot1.legend([str(round(btn2[10], 2)) + 'deg/s'], prop={'size':40}, handletextpad=0, handlelength =0)
                    self.plot1.legend([str(round(y[-1], 2)) + 'deg'], prop={'size':40}, handletextpad=0, handlelength =0)
                    self.canvas.draw()
        ...
        if __name__ == '__main__':
            with Manager() as manager:
                ...
                btn2 = manager.list([0, False, False, "", 1, False, 0, False, 0, 0, 0])
                info = manager.list([])
