    ...
    def Start2(self,user,day,MaxStretch):
        ...
        if __name__ == '__main__':
            with Manager() as manager:
                ...
                ####################[clicked and # cycles to display "keep on good job", Used to detect when reached goal, stop workout, value for messages, # set, check point to display message, set timer when velocity = 0, check point for vel timer, bit used to know if streatch failure has reached 3 times, # repetitions per set]
                btn2 = manager.list([0, False, False, "", 1, False, 0, False, 0, 0])
                info = manager.list([])
